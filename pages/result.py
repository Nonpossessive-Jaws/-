...
def render_stock_section(stock: dict, corp_name: str):
    """주가 정보 카드 + 새로고침 버튼"""
    from pages.listed import fetch_stock_info
...
def render_sector_section(stock: dict, benchmark: dict):
    ...
    # 현재 기업 영업이익률 계산 (data_by_item에서 가져올 수 없으니 yfinance 기준)
    # benchmark와 비교할 값은 result에서 가져옴
...
def render():
    ...
    if stock_info and (stock_info.get("섹터") or stock_info.get("업종") or sector_benchmark):
        ...
        if sector_benchmark:
            render_sector_section(stock_info, sector_benchmark)
        else:
            # 벤치마크 데이터 없어도 섹터/업종 배지는 표시
            render_sector_section(stock_info, {})
        st.divider()
